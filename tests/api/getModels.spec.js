...
test.describe("Get all models and model by ID", () => {
    let carsController
    ...
    test("Get models", async ()=>{
        ...
        expect(models.data).toEqual(expected)

    })

    test("Get model by ID", async ()=>{
        const getModel = await carsController.getModelById(1)
        const model = await getModel.json()
        ...
        expect(model.data).toEqual(expected)
    })
})